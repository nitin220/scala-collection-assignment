//Assignment on List -1

package com.knoldus.ManipulationOnStudentMarks
...
class MarksManipulation()
{
	...
	def namesWithPercentage(source_marks:List[List[List[Marks]]],source_student:List[Student]):List[(Double,String)]=
	{
		...
		val aggregate=for{
			...
			}yield if(check==0){folding_function(sort_list.slice(0,5))} else {folding_function(sort_list.slice((check*5),(check+1)*5))}
